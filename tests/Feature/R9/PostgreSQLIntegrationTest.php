...
class PostgreSQLIntegrationTest extends TestCase
{
    ...
    public function test_payment_persistence(): void{$user=User::factory()->create(); $wallet=Wallet::create(['user_id'=>$user->id,'currency'=>'BDT','balance_minor'=>0]); $payment=$this->createRow(Payment::class, ['user_id'=>$user->id,'wallet_id'=>$wallet->id,'provider'=>'manual','external_id'=>'ext-'.uniqid(),'amount_minor'=>1000,'currency'=>'BDT','status'=>Payment::STATUS_CREATED,'idempotency_key'=>'idem-'.uniqid()]); $this->assertDatabaseHas('payments',['id'=>$payment->id]);}
    ...
    public function test_settlement_uniqueness(): void{$tournament=Tournament::factory()->create(); $settlement=$this->createRow(FinancialSettlement::class, ['tournament_id'=>$tournament->id,'total_amount_minor'=>10000,'currency'=>'BDT','status'=>'pending','idempotency_key'=>'settlement-'.uniqid()]); $this->assertDatabaseHas('financial_settlements',['id'=>$settlement->id]); $this->expectException(\Illuminate\Database\QueryException::class); $this->createRow(FinancialSettlement::class, ['tournament_id'=>$tournament->id,'total_amount_minor'=>10000,'currency'=>'BDT','status'=>'pending','idempotency_key'=>$settlement->idempotency_key]);}
}
