...
class PostgreSQLConcurrencyTest extends TestCase
{
    ...
    public function test_concurrent_payout_same_idempotency_one_payout(): void{$user=User::factory()->create(); $tournament=Tournament::factory()->create(); $idemKey='payout-'.uniqid(); $this->createRow(Payout::class, ['user_id'=>$user->id,'tournament_id'=>$tournament->id,'amount_minor'=>5000,'currency'=>'BDT','status'=>'pending','external_id'=>'ext-'.uniqid(),'idempotency_key'=>$idemKey]); $this->expectException(\Illuminate\Database\QueryException::class); $this->createRow(Payout::class, ['user_id'=>$user->id,'tournament_id'=>$tournament->id,'amount_minor'=>5000,'currency'=>'BDT','status'=>'pending','external_id'=>'ext-'.uniqid(),'idempotency_key'=>$idemKey]);}
    public function test_concurrent_settlement_completion_once_only(): void{$tournament=Tournament::factory()->create(); $idemKey='settlement-'.uniqid(); $settlement=$this->createRow(FinancialSettlement::class, ['tournament_id'=>$tournament->id,'total_amount_minor'=>10000,'currency'=>'BDT','status'=>'pending','idempotency_key'=>$idemKey]); DB::transaction(function() use($settlement){$locked=FinancialSettlement::where('id',$settlement->id)->lockForUpdate()->first(); $this->updateRow($locked, ['status'=>'completed','completed_at'=>now()]);}); $settlement->refresh(); $this->assertEquals('completed',$settlement->status);}
}
