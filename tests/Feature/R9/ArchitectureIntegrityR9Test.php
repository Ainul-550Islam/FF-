...
class ArchitectureIntegrityR9Test extends TestCase
{
    ...
    public function test_no_financial_effect_bypasses_ledger(): void{$reflection=new \ReflectionClass(WalletService::class); $source=file_get_contents($reflection->getFileName()); $this->assertStringContainsString('ledger_entries',strtolower($source)); // Every financial movement is written through the guarded LedgerEntry model — either via Model::create() or, because the model is fully guarded, via explicit attribute assignment on a new instance.
