...
use Tests\TestCase;
use App\Models\User;
use App\Services\Gameberry\Stats\Stat13Service;
use Illuminate\Foundation\Testing\RefreshDatabase;
class Stat13Test extends TestCase
{
    use RefreshDatabase;
    protected Stat13Service $service;
    protected function setUp(): void { parent::setUp(); $this->service = app(Stat13Service::class); }
    public function test_stat_13_get_stats(): void
    {
        $user = User::factory()->create();
        $stats = $this->service->getStats($user->id);
        $this->assertEquals($user->id, $stats['user_id']);
        $this->assertEquals(13 * 100, $stats['stat_13_value']);
    ...
    public function test_stat_13_calculate(): void
    {
        $user = User::factory()->create();
        $result = $this->service->calculate($user->id, 100);
        $this->assertEquals(100 * 13 + $user->id, $result);
    }
}
