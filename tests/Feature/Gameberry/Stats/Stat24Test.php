...
class Stat24Test extends TestCase
{
    use RefreshDatabase;
    protected Stat24Service $service;
    protected function setUp(): void { parent::setUp(); $this->service = app(Stat24Service::class); }
    public function test_stat_24_get_stats(): void
    {
        $user = User::factory()->create();
        $stats = $this->service->getStats($user->id);
        $this->assertEquals($user->id, $stats['user_id']);
        $this->assertEquals(24 * 100, $stats['stat_24_value']);
    ...
    public function test_stat_24_calculate(): void
    {
        $user = User::factory()->create();
        $result = $this->service->calculate($user->id, 100);
        $this->assertEquals(100 * 24 + $user->id, $result);
    }
}
