...
class Stat18Test extends TestCase
{
    use RefreshDatabase;
    protected Stat18Service $service;
    protected function setUp(): void { parent::setUp(); $this->service = app(Stat18Service::class); }
    public function test_stat_18_get_stats(): void
    {
        $user = User::factory()->create();
        $stats = $this->service->getStats($user->id);
        $this->assertEquals($user->id, $stats['user_id']);
        $this->assertEquals(18 * 100, $stats['stat_18_value']);
    ...
    public function test_stat_18_calculate(): void
    {
        $user = User::factory()->create();
        $result = $this->service->calculate($user->id, 100);
        $this->assertEquals(100 * 18 + $user->id, $result);
    }
}
