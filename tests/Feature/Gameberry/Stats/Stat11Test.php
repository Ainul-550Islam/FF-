...
class Stat11Test extends TestCase
{
    use RefreshDatabase;
    protected Stat11Service $service;
    protected function setUp(): void { parent::setUp(); $this->service = app(Stat11Service::class); }
    public function test_stat_11_get_stats(): void
    {
        $user = User::factory()->create();
        $stats = $this->service->getStats($user->id);
        $this->assertEquals($user->id, $stats['user_id']);
        $this->assertEquals(11 * 100, $stats['stat_11_value']);
    ...
    public function test_stat_11_calculate(): void
    {
        $user = User::factory()->create();
        $result = $this->service->calculate($user->id, 100);
        $this->assertEquals(100 * 11 + $user->id, $result);
    }
}
