...
class Stat27Test extends TestCase
{
    use RefreshDatabase;
    protected Stat27Service $service;
    protected function setUp(): void { parent::setUp(); $this->service = app(Stat27Service::class); }
    public function test_stat_27_get_stats(): void
    {
        $user = User::factory()->create();
        $stats = $this->service->getStats($user->id);
        $this->assertEquals($user->id, $stats['user_id']);
        $this->assertEquals(27 * 100, $stats['stat_27_value']);
    ...
    public function test_stat_27_calculate(): void
    {
        $user = User::factory()->create();
        $result = $this->service->calculate($user->id, 100);
        $this->assertEquals(100 * 27 + $user->id, $result);
    }
}
