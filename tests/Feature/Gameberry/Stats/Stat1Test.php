...
class Stat1Test extends TestCase
{
    use RefreshDatabase;
    protected Stat1Service $service;
    protected function setUp(): void { parent::setUp(); $this->service = app(Stat1Service::class); }
    public function test_stat_1_get_stats(): void
    {
        $user = User::factory()->create();
        $stats = $this->service->getStats($user->id);
        $this->assertEquals($user->id, $stats['user_id']);
        $this->assertEquals(1 * 100, $stats['stat_1_value']);
    ...
    public function test_stat_1_calculate(): void
    {
        $user = User::factory()->create();
        $result = $this->service->calculate($user->id, 100);
        $this->assertEquals(100 * 1 + $user->id, $result);
    }
}
