...
class Stat9Test extends TestCase
{
    use RefreshDatabase;
    protected Stat9Service $service;
    protected function setUp(): void { parent::setUp(); $this->service = app(Stat9Service::class); }
    public function test_stat_9_get_stats(): void
    {
        $user = User::factory()->create();
        $stats = $this->service->getStats($user->id);
        $this->assertEquals($user->id, $stats['user_id']);
        $this->assertEquals(9 * 100, $stats['stat_9_value']);
    ...
    public function test_stat_9_calculate(): void
    {
        $user = User::factory()->create();
        $result = $this->service->calculate($user->id, 100);
        $this->assertEquals(100 * 9 + $user->id, $result);
    }
}
