...
class Stat14Test extends TestCase
{
    use RefreshDatabase;
    protected Stat14Service $service;
    protected function setUp(): void { parent::setUp(); $this->service = app(Stat14Service::class); }
    public function test_stat_14_get_stats(): void
    {
        $user = User::factory()->create();
        $stats = $this->service->getStats($user->id);
        $this->assertEquals($user->id, $stats['user_id']);
        $this->assertEquals(14 * 100, $stats['stat_14_value']);
    ...
    public function test_stat_14_calculate(): void
    {
        $user = User::factory()->create();
        $result = $this->service->calculate($user->id, 100);
        $this->assertEquals(100 * 14 + $user->id, $result);
    }
}
