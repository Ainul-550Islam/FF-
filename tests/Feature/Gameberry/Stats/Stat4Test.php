...
class Stat4Test extends TestCase
{
    use RefreshDatabase;
    protected Stat4Service $service;
    protected function setUp(): void { parent::setUp(); $this->service = app(Stat4Service::class); }
    public function test_stat_4_get_stats(): void
    {
        $user = User::factory()->create();
        $stats = $this->service->getStats($user->id);
        $this->assertEquals($user->id, $stats['user_id']);
        $this->assertEquals(4 * 100, $stats['stat_4_value']);
    ...
    public function test_stat_4_calculate(): void
    {
        $user = User::factory()->create();
        $result = $this->service->calculate($user->id, 100);
        $this->assertEquals(100 * 4 + $user->id, $result);
    }
}
