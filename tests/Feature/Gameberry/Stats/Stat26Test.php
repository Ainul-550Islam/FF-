...
class Stat26Test extends TestCase
{
    use RefreshDatabase;
    protected Stat26Service $service;
    protected function setUp(): void { parent::setUp(); $this->service = app(Stat26Service::class); }
    public function test_stat_26_get_stats(): void
    {
        $user = User::factory()->create();
        $stats = $this->service->getStats($user->id);
        $this->assertEquals($user->id, $stats['user_id']);
        $this->assertEquals(26 * 100, $stats['stat_26_value']);
    ...
    public function test_stat_26_calculate(): void
    {
        $user = User::factory()->create();
        $result = $this->service->calculate($user->id, 100);
        $this->assertEquals(100 * 26 + $user->id, $result);
    }
}
