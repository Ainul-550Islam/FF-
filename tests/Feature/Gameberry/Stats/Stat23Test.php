...
class Stat23Test extends TestCase
{
    use RefreshDatabase;
    protected Stat23Service $service;
    protected function setUp(): void { parent::setUp(); $this->service = app(Stat23Service::class); }
    public function test_stat_23_get_stats(): void
    {
        $user = User::factory()->create();
        $stats = $this->service->getStats($user->id);
        $this->assertEquals($user->id, $stats['user_id']);
        $this->assertEquals(23 * 100, $stats['stat_23_value']);
    ...
    public function test_stat_23_calculate(): void
    {
        $user = User::factory()->create();
        $result = $this->service->calculate($user->id, 100);
        $this->assertEquals(100 * 23 + $user->id, $result);
    }
}
