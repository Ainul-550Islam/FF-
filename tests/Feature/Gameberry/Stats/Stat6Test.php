...
class Stat6Test extends TestCase
{
    use RefreshDatabase;
    protected Stat6Service $service;
    protected function setUp(): void { parent::setUp(); $this->service = app(Stat6Service::class); }
    public function test_stat_6_get_stats(): void
    {
        $user = User::factory()->create();
        $stats = $this->service->getStats($user->id);
        $this->assertEquals($user->id, $stats['user_id']);
        $this->assertEquals(6 * 100, $stats['stat_6_value']);
    ...
    public function test_stat_6_calculate(): void
    {
        $user = User::factory()->create();
        $result = $this->service->calculate($user->id, 100);
        $this->assertEquals(100 * 6 + $user->id, $result);
    }
}
