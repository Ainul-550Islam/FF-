...
class Stat10Test extends TestCase
{
    use RefreshDatabase;
    protected Stat10Service $service;
    protected function setUp(): void { parent::setUp(); $this->service = app(Stat10Service::class); }
    public function test_stat_10_get_stats(): void
    {
        $user = User::factory()->create();
        $stats = $this->service->getStats($user->id);
        $this->assertEquals($user->id, $stats['user_id']);
        $this->assertEquals(10 * 100, $stats['stat_10_value']);
    ...
    public function test_stat_10_calculate(): void
    {
        $user = User::factory()->create();
        $result = $this->service->calculate($user->id, 100);
        $this->assertEquals(100 * 10 + $user->id, $result);
    }
}
