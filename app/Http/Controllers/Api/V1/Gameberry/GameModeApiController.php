...
class GameModeApiController extends Controller
{
    ...
    public function index() { return response()->json(['success' => true, 'data' => $this->modeService->getModes()]); }
    public function show(string $mode)
    {
        $data = $this->modeService->getMode($mode);
        if (!$data) return response()->json(['success' => false, 'error' => 'Mode not found'], 404);
