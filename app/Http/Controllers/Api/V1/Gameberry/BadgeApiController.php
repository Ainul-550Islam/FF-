...
class BadgeApiController extends Controller
{
    ...
    public function __construct(BadgeService $badgeService) { $this->badgeService = $badgeService; }
    ...
    {
        $badge = \App\Models\TitanBadge::with('league')->find($badgeId);
        if (!$badge) return response()->json(['success' => false, 'error' => 'Badge not found'], 404);
