...
use App\Services\Gameberry\Final8\Final1163Service;

class Final1198ApiController extends Controller
{
    protected $service;

    public function __construct()
    {
        $serviceClass = 'App\\Services\\Gameberry\\Final8\\Final1163Service';
    ...
    public function play(Request $request)
    {
        ...
        try {
            ...
            return response()->json([
                'success' => true,
                'data' => $result,
                'message' => 'Final8 1198 result: ' . ($result['is_win'] ? 'win' : 'loss') . ' - Gold at stake, magic chest, Level 4 Bronze unlock, reconciliation must hold STOP if mismatch G1 - Full file content no shortening',
                'feature' => 1198,
                'production_ready' => true,
                'no_shortening' => true,
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Final1198 play failed: ' . $e->getMessage() . ' - G1 Must Reconcile STOP',
                'feature' => 1198,
            ], 400);
        }
    }
