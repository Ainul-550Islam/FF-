...
class Core194ApiController extends Controller
{
    protected $service;
    ...
    public function index(Request $request)
    {
        ...
        return response()->json([
            ...
            'view' => 'gameberry.core.feature_' . 124,
    ...
    public function play(Request $request)
    {
        ...
        try {
            ...
            if (isset($result['reconcile']['all_balanced']) && !$result['reconcile']['all_balanced']) {
                Log::critical('G1 Financial totals must reconcile - STOP - Core194ApiController', [
                    'user_id' => $userId,
                    'result' => $result,
                ]);
            ...
            return response()->json([
                'success' => true,
                'data' => $result,
                'message' => 'Core Production 121-400 194 result: ' . ($result['is_win'] ? 'win' : 'loss') . ' - Gold at stake, magic chest, Level 4 Bronze unlock, reconciliation must hold STOP if mismatch G1 - Full file content no shortening',
                'feature' => $this->feature,
                'production_ready' => true,
                'no_shortening' => true,
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Core194 play failed: ' . $e->getMessage() . ' - G1 Must Reconcile STOP',
                'feature' => $this->feature,
            ], 400);
        }
    }
