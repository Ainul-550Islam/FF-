...
class Core311ApiController extends Controller
{
    protected $service;
    ...
    public function index(Request $request)
    {
        ...
        return response()->json([
            ...
            'view' => 'gameberry.core.feature_' . 215,
    ...
    public function play(Request $request)
    {
        ...
        try {
            ...
            if (isset($result['reconcile']['all_balanced']) && !$result['reconcile']['all_balanced']) {
                Log::critical('G1 Financial totals must reconcile - STOP - Core311ApiController', [
                    'user_id' => $userId,
                    'result' => $result,
                ]);
            ...
            return response()->json([
                'success' => true,
                'data' => $result,
                'message' => 'Core Production 121-400 311 result: ' . ($result['is_win'] ? 'win' : 'loss') . ' - Gold at stake, magic chest, Level 4 Bronze unlock, reconciliation must hold STOP if mismatch G1 - Full file content no shortening',
                'feature' => $this->feature,
                'production_ready' => true,
                'no_shortening' => true,
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Core311 play failed: ' . $e->getMessage() . ' - G1 Must Reconcile STOP',
                'feature' => $this->feature,
            ], 400);
        }
    }
