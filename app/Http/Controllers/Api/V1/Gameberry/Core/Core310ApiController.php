...
class Core310ApiController extends Controller
{
    protected $service;
    ...
    public function index(Request $request)
    {
        ...
        return response()->json([
            ...
            'view' => 'gameberry.core.feature_' . 214,
    ...
    public function play(Request $request)
    {
        ...
        try {
            ...
            if (isset($result['reconcile']['all_balanced']) && !$result['reconcile']['all_balanced']) {
                Log::critical('G1 Financial totals must reconcile - STOP - Core310ApiController', [
                    'user_id' => $userId,
                    'result' => $result,
                ]);
            ...
            return response()->json([
                'success' => true,
                'data' => $result,
                'message' => 'Core Production 121-400 310 result: ' . ($result['is_win'] ? 'win' : 'loss') . ' - Gold at stake, magic chest, Level 4 Bronze unlock, reconciliation must hold STOP if mismatch G1 - Full file content no shortening',
                'feature' => $this->feature,
                'production_ready' => true,
                'no_shortening' => true,
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Core310 play failed: ' . $e->getMessage() . ' - G1 Must Reconcile STOP',
                'feature' => $this->feature,
            ], 400);
        }
    }
