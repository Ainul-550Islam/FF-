...
class PrivateTableApiController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $myTables = \App\Models\PrivateTable::with(['participants.user'])->where('host_id', $userId)->where('status', '!=', 'expired')->orderByDesc('created_at')->get();
        $joinedTables = \App\Models\PrivateTable::with(['host'])->whereHas('participants', fn($q) => $q->where('user_id', $userId))->where('host_id', '!=', $userId)->where('status', '!=', 'expired')->orderByDesc('created_at')->get();
