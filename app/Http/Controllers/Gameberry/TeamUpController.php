...
class TeamUpController extends Controller
{
    ...
    public function joinTeam(Request $request, string $code)
    {
        ...
        $table = $this->tableService->getTableByCode($code);
        if (!$table) abort(404);
        if (!$table->is_team_up) return redirect()->back()->with('error','Not a team up table');
        $participant = \App\Models\PrivateTableParticipant::where('private_table_id', $table->id)->where('user_id', $userId)->first();
        if (!$participant) return redirect()->back()->with('error','Not a participant');
        if ($this->teamUpService->isTeamFull($table, $request->team)) return redirect()->back()->with('error','Team full');
