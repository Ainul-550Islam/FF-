...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $privateTablesCount = PrivateTable::where('host_id', $userId)->where('status', '!=', 'expired')->count();
        $buddiesCount = GameBuddy::where('user_id', $userId)->where('status', 'accepted')->count();
