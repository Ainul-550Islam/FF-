...
class Core354Controller extends Controller
{
    protected $service;
    ...
    {
        $this->feature = 354;
        $this->serviceNumber = '334';
        $this->view = 'gameberry.core.feature_' . 314;
    ...
    public function index(Request $request)
    {
        ...
        if (isset($stats['all_balanced']) && !$stats['all_balanced']) {
            Log::critical('G1 Financial totals must reconcile - STOP - Core354Controller', [
                'user_id' => $userId,
                'stats' => $stats,
            ]);
        }
    ...
    public function play(Request $request)
    {
        ...
        try {
            ...
            return redirect()->back()->with('success', 'Core Production 121-400 354 result: ' . ($result['is_win'] ? 'win' : 'loss') . ' - Gold at stake ' . $bet . ', win ' . $result['win_amount'] . ', balance ' . $result['gold_balance'] . ', magic chest, Level 4 Bronze unlock, reconciliation must hold STOP if mismatch G1 - Full file content no shortening');
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Core354 play failed: ' . $e->getMessage() . ' - G1 Must Reconcile STOP');
        }
    }
