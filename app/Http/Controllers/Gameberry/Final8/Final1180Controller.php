...
use App\Services\Gameberry\Final8\Final1160Service;

class Final1180Controller extends Controller
{
    protected $service;

    public function __construct()
    {
        $serviceClass = 'App\\Services\\Gameberry\\Final8\\Final' . (1160 ) . 'Service';
    ...
    public function index(Request $request)
    {
        ...
        if (isset($stats['all_balanced']) && !$stats['all_balanced']) {
            \Illuminate\Support\Facades\Log::critical('G1 Financial totals must reconcile - STOP - Final1180Controller', [
                'user_id' => $userId,
                'stats' => $stats,
            ]);
        ...
        return view('gameberry.final8.feature_' . (1110 ), compact('stats'));
    }
    ...
    public function play(Request $request)
    {
        ...
        try {
            ...
            return redirect()->back()->with('success', 'Final8 1180 result: ' . ($result['is_win'] ? 'win' : 'loss') . ' - Gold at stake ' . $bet . ', win ' . $result['win_amount'] . ', balance ' . $result['gold_balance'] . ', Level 4 Bronze unlock, reconciliation must hold STOP if mismatch G1 - Full file content no shortening');
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Final1180 play failed: ' . $e->getMessage() . ' - G1 Must Reconcile STOP');
        }
    }
