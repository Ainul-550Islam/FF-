...
class ReferralController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $scratchCards = \App\Models\ScratchCard::where('user_id', $userId)->orderByDesc('created_at')->get();
    ...
    {
        $userId = $request->user()->id;
        $cards = \App\Models\ScratchCard::where('user_id', $userId)->orderByDesc('created_at')->get();
    ...
    public function scratch(Request $request, int $cardId)
    {
        $userId = $request->user()->id;
        try {
            $card = \App\Models\ScratchCard::where('id', $cardId)->where('user_id', $userId)->firstOrFail();
            $rewards = $card->scratch();

            // Give rewards
            if ($rewards['gold'] > 0) {
                app(\App\Services\Gameberry\GoldEconomyService::class)->getOrCreateWallet($userId)->addGold($rewards['gold'], 'scratch_card', 'scratch_card', (string)$card->id, 'Scratch card reward');
            }
            if ($rewards['gems'] > 0) {
                app(\App\Services\Gameberry\GemEconomyService::class)->getOrCreateWallet($userId)->addGems($rewards['gems'], 'scratch_card', 'scratch_card', (string)$card->id, 'Scratch card gem reward');
            }
