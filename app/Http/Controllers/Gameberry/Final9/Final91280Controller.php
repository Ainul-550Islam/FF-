...
class Final91280Controller extends Controller
{
    protected $service;
    ...
    {
        $this->feature = 1280;
        $this->serviceNumber = '1260';
        $this->view = 'gameberry.final9.feature_' . 1210;
    ...
    public function index(Request $request)
    {
        ...
        if (isset($stats['all_balanced']) && !$stats['all_balanced']) {
            Log::critical('G1 Financial totals must reconcile - STOP - Final91280Controller', [
                'user_id' => $userId,
                'stats' => $stats,
            ]);
        }
    ...
    public function play(Request $request)
    {
        ...
        try {
            ...
            return redirect()->back()->with('success', 'Final9 Production 1200+ Full Code No Skip Existing Logic Preserved 1280 result: ' . ($result['is_win'] ? 'win' : 'loss') . ' - Gold at stake ' . $bet . ', win ' . $result['win_amount'] . ', balance ' . $result['gold_balance'] . ', magic chest, Level 4 Bronze unlock, reconciliation must hold STOP if mismatch G1 - Full file content no shortening');
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Final91280 play failed: ' . $e->getMessage() . ' - G1 Must Reconcile STOP');
        }
    }
