...
use App\Services\Gameberry\Final7\Final1054Service;

class Final1074Controller extends Controller
{
    protected $service;

    public function __construct()
    {
        $serviceClass = 'App\\Services\\Gameberry\\Final7\\Final' . (1054 ) . 'Service';
    ...
    public function index(Request $request)
    {
        ...
        if (isset($stats['all_balanced']) && !$stats['all_balanced']) {
            \Illuminate\Support\Facades\Log::critical('G1 Financial totals must reconcile - STOP - Final1074Controller', [
                'user_id' => $userId,
                'stats' => $stats,
            ]);
        ...
        return view('gameberry.final7.feature_' . (1004 ), compact('stats'));
    }
    ...
    public function play(Request $request)
    {
        ...
        try {
            ...
            return redirect()->back()->with('success', 'Final7 1074 result: ' . ($result['is_win'] ? 'win' : 'loss') . ' - Gold at stake ' . $bet . ', win ' . $result['win_amount'] . ', balance ' . $result['gold_balance'] . ', Level 4 Bronze unlock, reconciliation must hold STOP if mismatch G1 - Full file content no shortening');
        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Final1074 play failed: ' . $e->getMessage() . ' - G1 Must Reconcile STOP');
        }
    }
