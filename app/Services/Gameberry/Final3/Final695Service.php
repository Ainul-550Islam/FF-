...
class Final695Service
{
    ...
    public function play(int $userId, string $mode = 'classic', int $bet = 100): array
    {
        return DB::transaction(function () use ($userId, $mode, $bet) {
            ...
            $betTx = $goldService->placeBet($userId, $bet, 'FINAL3_695');
            $isWin = (bool) rand(0, 1);
            if ($isWin) {
                $goldService->winGold($userId, $bet * 2, 'FINAL3_695');
            ...
            return ['user_id' => $userId, 'mode' => $mode, 'bet' => $bet, 'is_win' => $isWin, 'level' => $level, 'league' => $league, 'chest' => $chest, 'reconcile' => $reconcile, 'bet_tx' => $betTx, 'feature_695' => true, 'full_code' => true];
        });
    }
}
