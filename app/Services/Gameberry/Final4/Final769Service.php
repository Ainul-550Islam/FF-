...
class Final769Service
{
    ...
    public function execute(int $userId, string $mode = 'classic', int $bet = 100): array
    {
        return DB::transaction(function () use ($userId, $mode, $bet) {
            ...
            $betTx = $goldService->placeBet($userId, $bet, 'FINAL4_769');
            $isWin = (bool) rand(0, 1);
            if ($isWin) {
                $goldService->winGold($userId, $bet * 2, 'FINAL4_769');
            ...
            return ['user_id' => $userId, 'mode' => $mode, 'bet' => $bet, 'is_win' => $isWin, 'level' => $level, 'league' => $league, 'chest' => $chest, 'reconcile' => $reconcile, 'bet_tx' => $betTx, 'feature_769' => true];
        });
    }
}
