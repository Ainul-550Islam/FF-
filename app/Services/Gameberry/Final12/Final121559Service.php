...
class Final121559Service
{
    ...
    public const FEATURE_NUMBER = 1559;
    ...
    public function getFullStats(int $userId): array
    {
        ...
        if (! $reconcile['all_balanced']) {
            Log::critical('G1 Financial totals must reconcile - STOP - Final121559Service', [
                'user_id' => $userId,
                'gold' => $reconcile['gold'],
                'gems' => $reconcile['gems'],
            ]);
    ...
    public function play(int $userId, string $mode = 'classic', int $bet = 100): array
    {
        ...
        return DB::transaction(function () use ($userId, $mode, $bet) {
            ...
            $betTx = $goldService->placeBet($userId, $bet, 'FINAL12_1559');
            $isWin = (bool) rand(0, 1);

            if ($isWin) {
                $winAmount = $bet * 2;
                $goldService->winGold($userId, $winAmount, 'FINAL12_1559');
            ...
            if (! $reconcile['all_balanced']) {
                Log::critical('Reconciliation failed STOP G1 - Final121559Service', [
                    'user_id' => $userId,
                    'gold' => $reconcile['gold'],
                    'gems' => $reconcile['gems'],
                ]);
