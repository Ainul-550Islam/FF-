...
class Final456Service
{
    ...
    public function processFullGameFlow(int $userId, string $gameMode = 'classic', int $betAmount = 100): array
    {
        return DB::transaction(function () use ($userId, $gameMode, $betAmount) {
            ...
            $betTx = $goldService->placeBet($userId, $betAmount, 'FINAL456_TABLE');
            $isWin = (bool) rand(0, 1);
            if ($isWin) {
                $winAmount = $betAmount * 2;
                $goldService->winGold($userId, $winAmount, 'FINAL456_TABLE');
            ...
            return ['user_id' => $userId, 'game_mode' => $gameMode, 'bet_amount' => $betAmount, 'result' => $result, 'level' => $level, 'league' => $league, 'chest' => $chest, 'reconcile' => $reconcile, 'bet_tx' => $betTx, 'feature_456' => true];
        });
    }
}
