...
class Core376Service
{
    ...
    public const FEATURE_NUMBER = 376;
    ...
    public function getFullStats(int $userId): array
    {
        ...
        if (! $reconcile['all_balanced']) {
            Log::critical('G1 Financial totals must reconcile - STOP - Core376Service', [
                'user_id' => $userId,
                'gold' => $reconcile['gold'],
                'gems' => $reconcile['gems'],
            ]);
    ...
    public function play(int $userId, string $mode = 'classic', int $bet = 100): array
    {
        ...
        return DB::transaction(function () use ($userId, $mode, $bet) {
            ...
            if (! $reconcile['all_balanced']) {
                Log::critical('Reconciliation failed STOP G1 - Core376Service', [
                    'user_id' => $userId,
                    'gold' => $reconcile['gold'],
                    'gems' => $reconcile['gems'],
                ]);
