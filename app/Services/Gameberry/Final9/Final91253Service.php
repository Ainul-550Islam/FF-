...
class Final91253Service
{
    ...
    public const FEATURE_NUMBER = 1253;
    ...
    public function getFullStats(int $userId): array
    {
        ...
        if (!$reconcile['all_balanced']) {
            Log::critical('G1 Financial totals must reconcile - STOP - Final91253Service', [
                'user_id' => $userId,
                'gold' => $reconcile['gold'],
                'gems' => $reconcile['gems'],
            ]);
    ...
    public function play(int $userId, string $mode = 'classic', int $bet = 100): array
    {
        ...
        return DB::transaction(function () use ($userId, $mode, $bet) {
            ...
            $betTx = $goldService->placeBet($userId, $bet, 'FINAL9_1253');
            $isWin = (bool) rand(0, 1);

            if ($isWin) {
                $winAmount = $bet * 2;
                $goldService->winGold($userId, $winAmount, 'FINAL9_1253');
            ...
            if (!$reconcile['all_balanced']) {
                Log::critical('Reconciliation failed STOP G1 - Final91253Service', [
                    'user_id' => $userId,
                    'gold' => $reconcile['gold'],
                    'gems' => $reconcile['gems'],
                ]);
