...
class Final576Service
{
    public function getStats(int $userId): array
    {
        ...
        return [
            'user_id' => $userId,
            'feature_576_value' => 576 * 100,
            'feature_576_growth' => 576 * 5,
    ...
    public function process(int $userId, string $mode = 'classic', int $bet = 100): array
    {
        return DB::transaction(function () use ($userId, $mode, $bet) {
            ...
            $betTx = $goldService->placeBet($userId, $bet, 'FINAL2_576');
            $isWin = (bool) rand(0,1);
            if ($isWin) {
                $goldService->winGold($userId, $bet*2, 'FINAL2_576');
            ...
            return ['user_id' => $userId, 'mode' => $mode, 'bet' => $bet, 'is_win' => $isWin, 'level' => $level, 'league' => $league, 'reconcile' => $reconcile, 'bet_tx' => $betTx, 'feature_576' => true];
        });
    }
}
