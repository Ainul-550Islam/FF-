...
class Final556Service
{
    public function getStats(int $userId): array
    {
        ...
        return [
            'user_id' => $userId,
            'feature_556_value' => 556 * 100,
            'feature_556_growth' => 556 * 5,
    ...
    public function process(int $userId, string $mode = 'classic', int $bet = 100): array
    {
        return DB::transaction(function () use ($userId, $mode, $bet) {
            ...
            $betTx = $goldService->placeBet($userId, $bet, 'FINAL2_556');
            $isWin = (bool) rand(0,1);
            if ($isWin) {
                $goldService->winGold($userId, $bet*2, 'FINAL2_556');
            ...
            return ['user_id' => $userId, 'mode' => $mode, 'bet' => $bet, 'is_win' => $isWin, 'level' => $level, 'league' => $league, 'reconcile' => $reconcile, 'bet_tx' => $betTx, 'feature_556' => true];
        });
    }
}
