...
class ChallengeService
{
    ...
    public function canChallenge(int $challengerId, int $challengedId): bool
    {
        ...
        if ($status && $status->hide_online_status) return false;
    ...
    {
        if (!$this->canChallenge($challengerId, $challengedId)) throw new \Exception('Cannot challenge - not buddies, hidden, or pending exists');
        if ($betAmount < self::MIN_BET || $betAmount > self::MAX_BET) throw new \Exception('Invalid bet amount');
        $goldService = app(GoldEconomyService::class);
        if (!$goldService->canAffordBet($challengerId, $betAmount)) throw new \Exception('Insufficient gold for challenge bet');
    ...
    public function acceptChallenge(int $challengeId, int $userId): Challenge
    {
        return DB::transaction(function () use ($challengeId, $userId) {
            ...
            if ($challenge->isExpired()) throw new \Exception('Challenge expired');
            $goldService = app(GoldEconomyService::class);
            if (!$goldService->canAffordBet($userId, $challenge->bet_amount_minor)) throw new \Exception('Insufficient gold to accept');
            $challenge->accept();
            return $challenge;
        });
    }
    ...
    {
        return Challenge::with(['challenger', 'challenged'])->where(function ($q) use ($userId) { $q->where('challenger_id', $userId)->orWhere('challenged_id', $userId); })->where('status', 'pending')->where('expires_at', '>', now())->orderByDesc('created_at')->get();
    }
}
