...
class Final958Service
{
    ...
    public function play(int $userId, string $mode = 'classic', int $bet = 100): array
    {
        return DB::transaction(function () use ($userId, $mode, $bet) {
            ...
            $betTx = $goldService->placeBet($userId, $bet, 'FINAL6_958');
            $isWin = (bool) rand(0,1);
            if ($isWin) {
                $goldService->winGold($userId, $bet*2, 'FINAL6_958');
            ...
            return ['user_id' => $userId, 'mode' => $mode, 'bet' => $bet, 'is_win' => $isWin, 'level' => $level, 'league' => $league, 'reconcile' => $reconcile, 'bet_tx' => $betTx, 'feature_958' => true, 'full_code' => true, 'no_shortening' => true];
        });
    }
}
