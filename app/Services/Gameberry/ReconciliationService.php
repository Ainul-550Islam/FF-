...
class ReconciliationService
{
    ...
    public function reconcileGold(int $userId): array
    {
        ...
        if (!$wallet) return ['is_balanced' => false, 'error' => 'Wallet not found'];
    ...
    public function reconcileAll(int $userId): array
    {
        ...
        if (!$allBalanced) {
            ...
            \Illuminate\Support\Facades\Log::critical("Reconciliation failed for user {$userId}", ['gold' => $gold, 'gems' => $gems]);
        }
    ...
    public function reconcileAllUsers(): array
    {
        ...
        foreach ($goldWallets as $wallet) {
            ...
            if ($result['all_balanced']) $results['balanced']++; else $results['unbalanced']++;
            if (!$result['all_balanced']) $results['details'][] = $result;
        }
        return $results;
    }
}
